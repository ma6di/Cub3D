...
/*   Updated: 2025/04/02 16:30:09 by mcheragh         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_rgb_str(char **rgb)
{
	...
	if (!is_valid_number(trim_whitespace(rgb[0])) || \
		!is_valid_number(trim_whitespace(rgb[1])) || \
		!is_valid_number(trim_whitespace(rgb[2])))
	{
...
static int	check_rgb(t_color *color)
{
	...
	if (!check_rgb_str(rgb))
		return (0);
	r = ft_atoi(trim_whitespace(rgb[0]));
	g = ft_atoi(trim_whitespace(rgb[1]));
	b = ft_atoi(trim_whitespace(rgb[2]));
