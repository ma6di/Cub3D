...
void render_sprites(t_game *game, t_sprite *sprites)
{
	...
	sprites->distance = pow(game->player.x - sprites->x, 2) +
								pow(game->player.y - sprites->y, 2);

// sort_sprites(game);  // ✅ Sort by distance (farther ones first)


	sprite_x = sprites->x - game->player.x;
	sprite_y = sprites->y - game->player.y;
...
{
	double half_size = 0.3; // ✅ Adjust based on sprite size
    ...
    if ((game->map[(int)(new_y - half_size)][(int)new_x] && \
		...
		game->map[(int)new_y][(int)(new_x + half_size)] == '1') || 
		(game->map[(int)(new_y - half_size)][(int)new_x] && \
		game->map[(int)(new_y - half_size)][(int)new_x] == 'D') || 
        (game->map[(int)(new_y + half_size)][(int)new_x] && \
		game->map[(int)(new_y + half_size)][(int)new_x] == 'D')|| 
        (game->map[(int)new_y][(int)(new_x - half_size)] && \
		game->map[(int)new_y][(int)(new_x - half_size)] == 'D') || 
        (game->map[(int)new_y][(int)(new_x + half_size)] && \
		game->map[(int)new_y][(int)(new_x + half_size)] == 'D'))
        return (0);  
