...
void render_key(t_game *game, t_key *key)
{
	...
	if (!key->active)  // Skip collected hearts
		return;
	...
	if (key->animation_timer >= 10)
	{
		...
		key->animation_timer = 0;  // Reset timer
	}
...
// sort_keys(game);  // ✅ Sort keys by distance (farther ones first)
	if (!key->active) // ✅ Skip collected keys
		return;
	sprite_x = game->key->x - game->player.x;
	sprite_y = game->key->y - game->player.y;
